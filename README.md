...
Use **Node.js 22.13.0 or later** and **pnpm 11.0.9**, as declared in `package.json`. pnpm is the supported package manager for local development and Docker builds; `pnpm-lock.yaml` is the only maintained dependency lockfile. npm and Yarn installs are not supported.

```bash
# Install the required pnpm version (npm only bootstraps the tool)
...
Use `pnpm add`, `pnpm remove`, or `pnpm update` when intentionally changing dependencies, and commit `package.json` together with `pnpm-lock.yaml`. Do not commit lockfiles generated by other package managers. If you previously installed dependencies with npm or Yarn, remove the existing `node_modules` directory before installing with pnpm.
